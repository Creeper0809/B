// path.b - Path utilities for v3.8

import std.io;
import std.str;

func path_dirname(path: u64, path_len: u64) -> u64 {
    var last_slash: u64 = 0 - 1;
    for (var i: u64 = 0; i < path_len; i++){
        if (*(*u8)(path + i) == 47) {
            last_slash = i;
        }
    }

    if (last_slash < 0) {
        var result: u64 = heap_alloc(2);
        *(*u8)result = 46;
        *(*u8)(result + 1) = 0;
        return result;
    }

    var result: u64 = heap_alloc(last_slash + 2);
    str_copy(result, path, last_slash);
    *(*u8)(result + last_slash) = 0;
    return result;
}

func path_join(dir: u64, dir_len: u64, name: u64, name_len: u64) -> u64 {
    var slash: u64 = heap_alloc(1);
    *(*u8)slash = 47;
    return str_concat3(dir, dir_len, slash, 1, name, name_len);
}

func module_to_path(name: u64, name_len: u64) -> u64 {
    var ext: u64 = heap_alloc(2);
    *(*u8)ext = 46;
    *(*u8)(ext + 1) = 98;
    return str_concat(name, name_len, ext, 2);
}
