...
const AST_DEREF = 106;
const AST_DEREF8 = 107;  // *(*u8)expr - byte dereference
const AST_CAST = 108;
...
func ast_program(funcs, consts, imports) {
    ...
    *(n + 16) = consts;
    *(n + 24) = imports;
    *(n + 32) = 0;  // globals (set by caller if needed)
    return n;
}
...
func parse_primary(p) {
    ...
    // Check for *(expr) or *(*u8)expr special syntax first
...
// Parse *(expr) or *(*u8)expr as special syntax
func parse_ptr_access(p) {
    ...
    if (ptr_kind > 0) {
        ...
        if (parse_peek_kind(p) == TOKEN_LBRACKET) {
            ...
            idx = parse_expr(p);
            parse_consume(p, TOKEN_RBRACKET);
            // For *(x), treat as *(x) - deref at x
            // For *(*u8)x, use AST_DEREF8
            if (ptr_kind == 64) {
                return ast_deref(idx);
            } else {
                // *(*u8)x - byte dereference
...
func symtab_new() {
    var s;
    s = heap_alloc(40);
    *(s) = vec_new(64);       // names (ptr to name structs)
    *(s + 8) = vec_new(64);   // stack offsets
    *(s + 16) = vec_new(64);  // types
    *(s + 24) = 0;            // count
    *(s + 32) = 0;            // current stack offset
    return s;
}
...
func string_get_label(str_ptr, str_len) {
    ...
    while (i < count) {
        ...
        if (str_eq(e_ptr, e_len, str_ptr, str_len)) {
            return *(entry + 16);  // Return existing label_id
        }
        i = i + 1;
    }
...
func get_expr_type(node) {
    ...
    if (kind == AST_CAST) {
        var result;
        result = heap_alloc(16);
        *(result) = *(node + 16);      // target_type
        *(result + 8) = *(node + 24);  // ptr_depth
        return result;
    }
    ...
    if (kind == AST_ADDR_OF) {
        ...
        if (op_type != 0) {
            var result;
            result = heap_alloc(16);
            *(result) = *(op_type);        // same base type
            *(result + 8) = *(op_type + 8) + 1;  // increase ptr_depth
            return result;
        }
    }
    ...
    if (kind == AST_DEREF) {
        ...
        if (op_type != 0) {
            ...
            if (depth > 0) {
                ...
                result = heap_alloc(16);
                *(result) = *(op_type);       // same base type
                *(result + 8) = depth - 1;        // decrease ptr_depth
                return result;
            }
        }
    }
...
func cg_expr(node) {
    ...
    // Byte dereference - *(*u8)x always reads 1 byte
    if (kind == AST_DEREF8) {
        var operand;
        operand = *(node + 8);
        cg_expr(operand);
...
func cg_stmt(node) {
    ...
    if (kind == AST_ASSIGN) {
        ...
        // Byte dereference write - *(*u8)x = val
...
func main(argc, argv) {
    ...
    *(merged_prog + 32) = g_all_globals;  // Set collected globals
