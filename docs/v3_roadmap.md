	...
	- [ ] `var` 로컬 선언
		...
		- [ ] 레지스터 별칭: `alias <reg>: <name> [= expr];`
			- [ ] v3 MVP(권장): x86-64 GPR 전체를 별칭 대상으로 확장
				...
				- [ ] `rsp`는 금지(스택 포인터 파손 위험), `rbp`는 정책으로 결정(권장: 금지 또는 제한)
...
- [ ] (추천) 상수시간 선택(constant-time select): `ct_select(mask, a, b)`
	...
	- [ ] 의미(정의): `mask`가 all-ones면 `a`, all-zeros면 `b`를 반환
		- [ ] MVP mask 규칙(택1): (A) `mask: u64`는 `0` 또는 `~0`만 허용, (B) 임의 값 허용 시 `((a & mask) | (b & ~mask))`로 정의
...
- [ ] 다차원 고정 배열(MVP 권장: 컴파일 타임 크기만)
	- [ ] 문법(안): `[N][M]T` 처럼 중첩 배열 타입을 허용(= "배열의 배열")
		- [ ] 예: `var a: [3][4]u8;` / `var a[3][4];`(기존 v2 스타일을 확장할지 여부는 택1)
