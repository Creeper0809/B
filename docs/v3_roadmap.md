...
### 1.7.5) 부동소수점 타입 (v4 필수)

**v4 로드맵 구현에 필수**: GPU 벡터 연산, 조건부 컴파일 예제 등에서 부동소수점이 필요하므로 v3에서 구현.
...
- [ ] 산술 연산
    - [ ] `+`, `-`, `*`, `/` (기본 산술)
    - [ ] 부동소수점 리터럴 파싱: `3.14`, `1.0e-5`, `0.5f` 등
- [ ] 비교 연산
    - [ ] `<`, `>`, `<=`, `>=`, `==`, `!=`
    - [ ] NaN 처리 정책 명시 (IEEE 754 표준 동작)
- [ ] 타입 변환
    - [ ] `cast(f32, int_val)` (정수 → 부동소수점)
    - [ ] `cast(u64, float_val)` (부동소수점 → 정수, 잘림)
- [ ] 코드젠
    - [ ] x86-64 SSE2 명령어 사용 (`addss`, `mulss`, `cvtsi2ss` 등)
    - [ ] XMM 레지스터 관리 (xmm0~xmm15)
- DoD
    - `var x: f32 = 3.14;` 선언/초기화 동작
    - `var y = x + 1.5;` 산술 연산 동작
    - 정수 타입과 혼용 시 명시적 `cast` 필요 (타입 안전)

### 1.8) 일급 함수: 함수 포인터 & (선택) 클로저

**v4 로드맵 구현에 필수**: 콜백 패턴, 고차 함수, 리플렉션 고급 기능 등에서 타입 안전한 함수 포인터가 필요하므로 v3에서 구현.
...
- DoD
    - `var cmp: func(i32, i32) -> bool = my_compare;` 선언 동작
    - `var result = cmp(10, 20);` 간접 호출 동작
    - 제네릭과 조합하여 `sort<T>(arr: []T, cmp: func(T, T) -> bool)` 패턴 가능
...
## 3) 제네릭(Generics) - Phase 5.1 (Updated)

**목표:** 표준 라이브러리(`Vec`, `Map`)와 유틸리티 함수 구현을 위한 필수 기능.  
**전략:** **AST 단계 모노모피즘(AST-Level Monomorphization)**

제네릭은 런타임 오버헤드가 없어야 하며(C++ 템플릿 방식), IR이나 백엔드까지 제네릭 정보를 가져가지 않는다.
프론트엔드에서 타입별로 코드를 복제(monomorphization)하여 해결한다.

### 3.1) 문법(Syntax)

Go 스타일(`[]`)보다는 시스템 프로그래밍의 관습인 **C++ 스타일(`< >`)**을 채택.
이는 Basm v3의 정체성(C++의 계승자)과 `k.<Player>.hp` 문법과의 일관성에 부합한다.
...
var v: Vec<u64>;
var x = id<i32>(10);
var y = id(20);  // 타입 추론 (T=i32)
```

### 3.2) 구현 로드맵(Phase 5.1)

- [ ] 1. 파서(Parser): 룩어헤드(Lookahead) 구현
    - [ ] **난이도:** 상
    - [ ] **이슈:** `f<T>(...)`와 `a < b`(비교 연산) 구분 필요
    - [ ] **해결:** `<`를 만났을 때, 뒤따르는 토큰들을 미리 읽어보고(lookahead) 타입 패턴이면 제네릭으로 파싱
    - [ ] 파싱 규칙:
        - [ ] 제네릭 선언: `func name<T, U>(...)`
        - [ ] 제네릭 타입: `struct name<T> { ... }`
        - [ ] 제네릭 인스턴스화: `Type<Args>`, `func<Args>(...)`

- [ ] 2. AST 정의 및 등록(Registration)
    - [ ] **GenericDecl 노드:** 제네릭 함수/구조체는 바로 컴파일(CodeGen)하지 않고, **"청사진(Template)"** 상태로 별도 저장소(Registry)에 보관
    - [ ] `func id<T>` → AST Registry에 등록만 하고 넘어감
    - [ ] Registry 구조: `HashMap<name, GenericTemplate>`
        - [ ] Template: 원본 AST + 타입 파라미터 목록

- [ ] 3. 인스턴스화(Instantiation) - 핵심
    - [ ] **트리거:** `id<u64>(...)` 호출 또는 `Vec<u64>` 타입 사용이 감지됨
    - [ ] **동작:**
        1. [ ] **Lookup:** 레지스트리에서 `id` 청사진을 가져옴
        2. [ ] **Deep Copy:** AST 전체를 깊은 복사함
        3. [ ] **Substitution:** 복사된 AST 내의 모든 `T` 노드를 `u64` 노드로 교체
        4. [ ] **Name Mangling:** 함수 이름을 `id_u64_xxhash` 처럼 유니크하게 변경
        5. [ ] **Injection:** 생성된 구체적인 함수(`id_u64`)를 AST 파이프라인에 주입하여 컴파일 진행
    - [ ] **캐싱:** 이미 `id<u64>`가 생성되어 있다면, 다시 만들지 않고 재사용(Memoization)
    - [ ] **심볼 관리:** Mangled name으로 심볼 테이블에 등록하여 충돌 방지

- [ ] 4. 타입 추론(Type Inference)
    - [ ] `id(10)` 호출 시, 인자 `10`의 타입(`i32` 등)을 보고 `T=i32`라고 추론하여 `id<i32>(10)`으로 자동 변환
    - [ ] 추론 규칙:
        - [ ] 인자 타입에서 타입 파라미터 추론
        - [ ] 반환 타입에서 타입 파라미터 추론(필요 시)
        - [ ] 추론 실패 시 명시적 타입 인자 요구(컴파일 에러)

### 3.3) 제약 조건(Constraints) - Phase 5.2로 연기

MVP에서는 제약 조건(`trait`) 없이 구현한다.

- [ ] **Compile-time Duck Typing:**
    - [ ] C++ 템플릿처럼 일단 타입을 넣어서 인스턴스화함
    - [ ] 만약 `T`가 `+` 연산을 지원하지 않는데 `x + y`를 했다면? → **그 줄에서 컴파일 에러 발생**
    - [ ] 장점: 구현이 단순함(타입 체크만으로 해결)
    - [ ] 단점: 에러 메시지가 제네릭 사용처가 아니라 인스턴스화된 코드에서 발생

- [ ] Phase 5.2에서 추가(선택):
    - [ ] `trait Eq { ... }` 정의
    - [ ] `func f<T: Eq>(x: T) -> bool` 제약 문법
    - [ ] 타입 체크 시점에 trait 만족 여부 검증

### 3.4) Definition of Done(DoD)

- [ ] `struct Vec<u64>` 선언 시 메모리 레이아웃이 `ptr: *u64`로 잡혀야 함
- [ ] `func add<T>(a: T, b: T)`를 `add(10, 20)`으로 호출했을 때, `add_i32`가 생성되고 실행되어야 함
- [ ] 서로 다른 파일에서 `Vec<i32>`를 써도, 심볼 충돌 없이 링크되어야 함
- [ ] 동일 제네릭 인스턴스(`Vec<u64>`)가 여러 번 사용되어도 한 번만 생성되어야 함(캐싱)
- [ ] 제네릭 함수 내부에서 타입 파라미터 `T`를 사용한 연산(`sizeof(T)`, `*T` 등)이 정상 동작해야 함
