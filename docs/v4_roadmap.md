...
- [x] **내장 함수**: print/println, panic, sizeof, offsetof, cast
- [x] **인라인 어셈블리**: `asm { ... }` 블록
...
- [ ] **`match`**: 패턴 매칭 표현식 (모든 타입 지원, `switch` 대체)
  ...
  **명시적 최적화 제어** (해커용):
  ...
  @[jump_table]
  match byte_opcode {
      0...255 => handle_instruction(byte_opcode),
  }
  ```
...
- [ ] **`enum` with data**: variant enum (Sum Type)
  ```b
  enum Option<T> {
      Some(T),
      None,
...
**참고**: `delete` 키워드는 도입하지 않습니다. `new`는 `malloc`의 설탕일 뿐이므로, 
해제는 `free`를 사용하는 것이 메모리 모델의 일관성을 유지합니다.
...
**제약**:
- 생성자는 **자동 호출되지 않음**: `var x: T;`는 생성자 호출 안 함 (Zero-initialized만)
- 소멸자는 **자동 호출되지 않음**: 명시적 `delete` 또는 `defer delete`만 가능
...
- [ ] **trait 상속**: 구조체가 trait를 구현하면 VTable 포인터 자동 추가
  ```b
  ...
  // 컴파일러가 자동 변환:
  // struct Circle {
  //     $vptr: *DrawableVTable;  // Offset 0 (숨겨진 필드)
  //     radius: f64;              // Offset 8
  // }
  ```
...
**철학**:
- **명시적 마법**: 리터럴 초기화 시에만 vptr 설정
- **Zero-Cost 상속**: trait 없는 상속은 vptr 오버헤드 없음
- **해커 친화적**: `var raw: Circle;`은 순수 메모리 할당
...
#### **제외된 기능** (B 철학과 맞지 않음)
- ~~`let`/`mut`~~: Rust 소유권 시스템, B는 `var`/`const` 사용
- ~~`move`/`drop`/`pin`~~: Rust 소유권, B는 명시적 메모리 관리
- ~~`when`/`unless`~~: Ruby 문법 설탕, B는 명시성 우선
- ~~`async`/`await`~~: v5로 미룸 (복잡도 높음)
- ~~`yield`~~: 제너레이터는 v5
- ~~`thread_local`~~: 동시성은 v5
- ~~`trait`~~: 다중 상속으로 대체 가능, 정적 디스패치만 사용
- ~~`is`/`as`~~: `cast`로 충분
- ~~범위 연산자 `..`/`..=`~~: 기존 `for` 루프로 충분
...
- [ ] **String** 타입 (UTF-8)
  ...
  for (var ch in s.chars()) {  // char iterator
      println(ch);
  }
  ```
...
**표준 라이브러리 기본 확장**:
- [ ] **String 타입** (UTF-8): `String.from()`, `push_str()`, `chars()`
- [ ] **Range 타입**: `0..10`, `0..=10`
