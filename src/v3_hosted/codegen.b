...
// NOTE: wipe/secret features removed in v3 simplification (moved to v4)
...
// NOTE: Property hooks (@[getter]/@[setter]) removed in v3 simplification (moved to v4)

// Helper function to check if AST has a function with given name
// (used for function pointer support)
...
func cg_lower_expr(ctx, e) {
	...
	if (ek == AstExprKind.FIELD) {
		...
		// NOTE: Property hooks removed in v3 simplification (moved to v4)
		// All field access is now direct (no getter/setter calls)
		if (sz == 0) {
	...
	if (ek == AstExprKind.IDENT) {
		...
		var alias_reg = cg_alias_lookup(ctx, name_ptr, name_len);
		if (alias_reg != 0 - 1) {
			// It's an alias - emit register load
			ir_emit(f, IrInstrKind.ALIAS_LOAD, alias_reg, 0, 0);
			return 0;
		}
		var l = cg_local_find(ctx, name_ptr, name_len);
		if (l == 0) {
	...
	if (ek == AstExprKind.BINARY) {
		...
		if (op == TokKind.EQ) {
			...
			var alias_reg = cg_alias_lookup(ctx, name_ptr, name_len);
			if (alias_reg != 0 - 1) {
				// It's an alias - evaluate rhs and store to register
				cg_lower_expr(ctx, rhs);
				ir_emit(f, IrInstrKind.ALIAS_STORE, alias_reg, 0, 0);
				// Don't push a value - assignment result will be discarded anyway
				// and EXPR stmt will pop one value. Push dummy 0 to balance.
				ir_emit(f, IrInstrKind.PUSH_IMM, 0, 0, 0);
				return 0;
			}
			var l = cg_local_find(ctx, name_ptr, name_len);
...
// Alias registry: maps alias names to register IDs
// Simple approach: store in ctx at offset 96 as Vec of (name_ptr, name_len, reg_id)
func cg_alias_register(ctx, reg_ptr, reg_len, name_ptr, name_len) {
	var aliases = ptr64[ctx + 96];
	if (aliases == 0) {
		aliases = vec_new(4);
		ptr64[ctx + 96] = aliases;
	}
	// Store as triplet: name_ptr, name_len, reg_id
	var reg_id = ir_parse_reg_name(reg_ptr, reg_len);
	var entry = heap_alloc(24);
	if (entry == 0) { return 0; }
	ptr64[entry + 0] = name_ptr;
	ptr64[entry + 8] = name_len;
	ptr64[entry + 16] = reg_id;
	vec_push(aliases, entry);
	return 0;
}

func cg_alias_lookup(ctx, name_ptr, name_len) {
	// Returns reg_id or -1 if not found
	var aliases = ptr64[ctx + 96];
	if (aliases == 0) { return 0 - 1; }
	var n = vec_len(aliases);
	var i = 0;
	while (i < n) {
		var entry = vec_get(aliases, i);
		var ep = ptr64[entry + 0];
		var el = ptr64[entry + 8];
		if (slice_eq_parts(name_ptr, name_len, ep, el) == 1) {
			return ptr64[entry + 16];
		}
		i = i + 1;
	}
	return 0 - 1;
...
func cg_lower_stmt(ctx, st) {
	...
	if (k == AstStmtKind.EXPR) {
		var e = ptr64[st + 56];
		cg_lower_expr(ctx, e);
		// Use DROP instead of POP to preserve alias registers
		if (cg_expr_is_slice(ctx, e) == 1) {
			ir_emit(f, IrInstrKind.DROP, 0, 0, 0);
			ir_emit(f, IrInstrKind.DROP, 0, 0, 0);
		}
		else {
			ir_emit(f, IrInstrKind.DROP, 0, 0, 0);
	...
	if (k == AstStmtKind.RETURN) {
		...
		// NOTE: secret zeroization removed in v3 simplification (moved to v4)
		var e = ptr64[st + 56];
	...
	if (k == AstStmtKind.BLOCK) {
		...
		// NOTE: secret vars removed in v3 simplification (moved to v4)
	...
	// NOTE: WIPE statement removed in v3 simplification (moved to v4)
	if (k == AstStmtKind.WIPE) {
		// wipe is no longer supported in v3
		return 0;
	}
	...
	if (k == AstStmtKind.ASM) {
		// a = body ptr, b = body len
		var body_ptr = ptr64[st + 8];
		var body_len = ptr64[st + 16];
		ir_emit_asm(f, body_ptr, body_len);
		return 0;
	}
	if (k == AstStmtKind.ALIAS) {
		// alias reg : name [= init];
		// a = reg name ptr, b = reg name len
		// name_ptr = name_ptr, name_len = name_len
		// expr_ptr = init expr or 0
		var reg_ptr = ptr64[st + 8];
		var reg_len = ptr64[st + 16];
		var name_ptr = ptr64[st + 32];
		var name_len = ptr64[st + 40];
		var init_expr = ptr64[st + 56];
		// Register the alias in codegen context
		cg_alias_register(ctx, reg_ptr, reg_len, name_ptr, name_len);
		// If init expression exists, evaluate and store
		if (init_expr != 0) {
			cg_lower_expr(ctx, init_expr);
			ir_emit_alias_store(f, reg_ptr, reg_len);
		}
		return 0;
	}
...
func cg_emit_reg_name(sb, reg_id) {
	// reg_id: 0=rax, 1=rbx, 2=rcx, 3=rdx, 4=rsi, 5=rdi, 6=r8, ...
	if (reg_id == 0) { cg_emit(sb, "rax"); }
	else if (reg_id == 1) { cg_emit(sb, "rbx"); }
	else if (reg_id == 2) { cg_emit(sb, "rcx"); }
	else if (reg_id == 3) { cg_emit(sb, "rdx"); }
	else if (reg_id == 4) { cg_emit(sb, "rsi"); }
	else if (reg_id == 5) { cg_emit(sb, "rdi"); }
	else if (reg_id == 6) { cg_emit(sb, "r8"); }
	else if (reg_id == 7) { cg_emit(sb, "r9"); }
	else if (reg_id == 8) { cg_emit(sb, "r10"); }
	else if (reg_id == 9) { cg_emit(sb, "r11"); }
	else if (reg_id == 10) { cg_emit(sb, "r12"); }
	else if (reg_id == 11) { cg_emit(sb, "r13"); }
	else if (reg_id == 12) { cg_emit(sb, "r14"); }
	else if (reg_id == 13) { cg_emit(sb, "r15"); }
	else { cg_emit(sb, "rax"); }
	return 0;
}
...
func cg_gen_asm(prog, ir_func) {
	...
	while (fi < fnc) {
		...
		while (i < n) {
			...
			else if (k == IrInstrKind.ASM_RAW) {
				// a=body_ptr, b=body_len: emit raw assembly
				cg_emit_bytes(sb, a, b);
				cg_emit_nl(sb);
			}
			else if (k == IrInstrKind.ALIAS_LOAD) {
				// a=reg_id: push register value to stack
				cg_emit(sb, "\tpush ");
				cg_emit_reg_name(sb, a);
				cg_emit_nl(sb);
			}
			else if (k == IrInstrKind.ALIAS_STORE) {
				// a=reg_id: pop stack value to register
				cg_emit(sb, "\tpop ");
				cg_emit_reg_name(sb, a);
				cg_emit_nl(sb);
			}
			else if (k == IrInstrKind.DROP) {
				// Discard stack top without modifying registers (for alias support)
				cg_emit_line(sb, "\tadd rsp, 8");
			}
