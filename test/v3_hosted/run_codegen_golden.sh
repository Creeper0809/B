...
golden_dir="$root_dir/test/v3_hosted/codegen_golden"
...
for input in "$golden_dir"/*.b; do
  ...
  asm="$out_dir/${base}.v3h.asm"
  obj="$out_dir/${base}.v3h.o"
  bin="$out_dir/${base}.v3h.bin"

  # Generate asm
  set +e
  timeout 5s "$smoke_bin" "$input" >"$asm"
  gen_status=$?
  set -e
  if [[ $gen_status -ne 0 ]]; then
    echo "FAIL: asm generation: $base (status $gen_status)" >&2
    fail=$((fail+1))
    continue
  fi
  ...
  # Run
  stdout="$out_dir/${base}.stdout"
  : >"$stdout"
  set +e
  timeout 5s "$bin" >"$stdout"
  status=$?
  set -e
  ...
  exp_out="$golden_dir/${base}.stdout"
  exp_status="$golden_dir/${base}.status"

  if ! diff -u "$exp_out" "$stdout" >/dev/null; then
    echo "FAIL: stdout mismatch: $base" >&2
    diff -u "$exp_out" "$stdout" >&2 || true
    fail=$((fail+1))
    continue
  fi
...
echo "codegen_golden: ok=$ok fail=$fail"
[[ $fail -eq 0 ]]
